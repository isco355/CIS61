    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if self.ant is insect:
                if getattr(insect,'ant',None) is not None:
                    self.ant=insect.ant
                else:
                    self.ant = None
            elif getattr(self.ant,'ant',None)==insect:
                self.ant.ant=None
            else:
...
class Ant(Insect):
    ...
    def can_contain(self,other):
        condition = self.container and (self.ant is None) and (not other.container)
        return condition
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor-=amount
        if self.armor <=0:
            duplicate_bees= self.place.bees.copy()
...
class QueenPlace:
    ...
    def bees(self):
        "*** YOUR CODE HERE ***"
        return self.colony_queen.bees + self.ant_queen_place.bees
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        self.doubleTunnelAnts()

    def doubleTunnelAnts(self):
        current=self.place
        while current:
            self.doubleAnt(current.ant)
    ...
    def doubleAnt(self,ant):
        if ant is None or ant is self:
            return
        
        if ant.container and ant.ant:
            if ant.ant not in self.double_ants:
                ant.ant.damage*=2
                self.double_ants.append(ant.ant)
...
def make_stun(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def stunnedAction(colony):
        pass
    return stunnedAction
...
def apply_effect(effect, bee, duration):
    ...
    def newAction(colony):
        nonlocal counter,original_action,affected_action
        if counter >0:
