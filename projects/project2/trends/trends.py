...
def convertToASCIILetterWord(text):
    only_letters=[char  if char.isalpha() else " " for char in text]
    ascii_letter=''.join(only_letters)
    return  ascii_letter.split()
...
def extract_words(text):
    ...
    for word in split_words:
        ascii_word=convertToASCIILetterWord(word)
        clean_words.extend(ascii_word)
    return   clean_words
...
def make_sentiment(value):
    ...
    def sentiment(attribute):
        if attribute == 'has_sentiment':
            return value is not None
        elif attribute == 'sentiment_value':
            return value
...
def analyze_tweet_sentiment(tweet):
    ...
    sentiment_words = [get_word_sentiment(w) for w in words]
    sentiment_total = [ sentiment_value(word_sentiment)  for word_sentiment in sentiment_words if has_sentiment(word_sentiment)]

    total_sentiments_words=len(sentiment_total)
    if total_sentiments_words !=0 :

        average_sentiment = sum(sentiment_total)/total_sentiments_words
...
def find_centroid(polygon):
    ...
    for i in range(n - 1):
        point_a = polygon[i]
        point_b = polygon[i+1]
        x0, y0 = latitude(point_a), longitude(point_a)
        x1, y1 = latitude(point_b), longitude(point_b)

        sub_area = x0 * y1 - x1 * y0
        area += sub_area
        center_x += (x0 + x1) * sub_area
        center_y += (y0 + y1) * sub_area

    area = area* 0.5
    if area == 0:
        first_point=polygon[0]
        return latitude(first_point), longitude(first_point), 0
...
def find_state_center(polygons):
    ...
    center_lat = total_lat / total_area
    center_lon=total_lon / total_area
    center_position =make_position(center_lat,center_lon )
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        closest_state= min(
            state_centers,
            key=lambda state: geo_distance(tweet_position, state_centers[state])
        )
...
def average_sentiments(tweets_by_state):
    ...
    for state, tweets in tweets_by_state.items():
        ...
        sentiment_tweets=[ analyze_tweet_sentiment(tweet) for tweet in tweets ]
        sentiment_values =[ sentiment_value(sentiment)  for sentiment in sentiment_tweets if has_sentiment(sentiment)] 
        if sentiment_values:
